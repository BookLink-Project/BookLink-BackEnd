...
//    MessageRoom findMessageRoomBySenderAndReceiver(@Param("sender") Member sender, @Param("receiver") Member receiver);
    ...
    @Query("SELECT mr FROM MessageRoom mr WHERE (:member IN (mr.sender, mr.receiver)) AND (:otherMember IN (mr.sender, mr.receiver))")
    MessageRoom findMessageRoomByMembers(@Param("member") Member member, @Param("otherMember") Member otherMember);
